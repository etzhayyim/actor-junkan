...
- `data/ontology/ontology.junkan-gov.edn` — EAVT schema · 5 asymmetry stocks
...
- `data/ontology/ontology.junkan-demography.edn` — EAVT schema · 5 demographic stocks
...
- `data/seed/seed.low-fertility-societies.edn` — peer low-fertility societies for
...
- `test/junkan/methods/test_demography.cljc` — 12 tests (contribution sign · clean validate ·
  ...
  datom discipline). Wired into `junkan.test-runner`. Full suite: **77 tests / 6443
  assertions green**.
...
- `src/junkan/methods/waste_sanitation.cljc` — analysis-only read-off (8 stocks: collection-
  ...
  **No outward channel (G4 by absence).**
- `data/seed/seed.india-waste-sanitation.edn` — 12 representative signals across all 7
...
Actor methods under `src/junkan/methods/` (R0 path-reserved,
