(ns junkan.methods.test-charter-gates
  ...
  Reads canonical EDN lexicons with string keys. It weakens no gate; it asserts them.
...
#?(:clj
   (do
     ...
     (def ^:private lexdir (java.io.File. actor-dir "data/ontology"))
     (defn- lex [name]
       (edn/read-string (slurp (java.io.File. lexdir (str name ".wire.edn")))))
     (defn- manifest []
